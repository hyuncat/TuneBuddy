...
class MidiPlayer:
    ...
    def __init__(self, midi_synth: MidiSynth):
    ...
    def _play(self, start_time: float=0):
        ...
        for _, program_change_messages in self.midi_data.program_dict.items():
            self.midi_synth.handle_midi(program_change_messages[0])
        
        msg_times = list(self.midi_data.message_dict.keys())
        ...
        for i in range(start_idx, len(msg_times)):
            ...
            messages = self.midi_data.message_dict[current_time]

            for msg in messages:
                # only handle messages in active_channels
                if msg.channel in self.active_channels:
