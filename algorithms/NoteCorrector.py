...
class NoteCorrector:
    ...
    def string_edit(cls, user_string: UserString, midi_string: MidiString):
        ...
        mat[0, :] = np.cumsum([0]+[NoteCorrector.INSERTION_COST]*M) # all insertions
        mat[:, 0] = np.cumsum([0]+[NoteCorrector.DELETION_COST]*N) # all deletions
        ...
        for i in range(1, N+1):
            for j in range(1, M+1):
                ...
                NoteCorrector.SUBSTITUTION_COST = 1
                note_distance = user_string.get_distance(midi_string, j-1, i-1)
                if abs(note_distance) < NoteCorrector.TOLERANCE: # being generous, the NoteCorrector.TOLERANCE
                    NoteCorrector.SUBSTITUTION_COST = 0 # same note pitch

                top_three = np.array([
                    top + NoteCorrector.DELETION_COST,
                    diag + NoteCorrector.SUBSTITUTION_COST,
                    left + NoteCorrector.INSERTION_COST
                ])
        ...
        while i>0 or j>0:
            ...
            if mistake_type==1 and i>0 and j>0:
                note_distance = user_string.get_distance(midi_string, j-1, i-1)
                if abs(note_distance) >= NoteCorrector.TOLERANCE:
                    mistakes.append(
